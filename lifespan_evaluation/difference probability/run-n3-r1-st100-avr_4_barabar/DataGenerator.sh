...
SEED=10
N_NODES=3
CHANNEL_EXISTANCE_PROB=$(python -c "print(.7)")
...
AVERAGE_RATE=$(python -c "print(1)")
STD_RATE=$(python -c "print(1)")
...
python mrates_generator.py $SEED $N_NODES $SPARSE_COEF $AVERAGE_RATE $STD_RATE

n_channels=$(python network_generator.py $SEED $N_NODES $CHANNEL_EXISTANCE_PROB $AVERAGE_CAPACITY $AVERAGE_PAYMENT_AMOUNT $MRATES_FILE $FEE_BASE $FEE_PROPORTIONAL $MIN_HTLC $TIMELOCK)
...
do
    ...
    python payment_generator.py $(($SEED + $i)) $SIMULATION_TIME $MRATES_FILE $AVERAGE_PAYMENT_AMOUNT $PAYMENT_AMOUNT_STD
