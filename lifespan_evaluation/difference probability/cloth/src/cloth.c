...
void write_output(struct network* network, struct array* payments, char output_dir_name[]) {
  ...
  fprintf(csv_channel_output, "id,edge1,edge2,node1,node2,capacity,is_closed,is_unbalanced,unbalancing_time\n");
  for(i=0; i<array_len(network->channels); i++) {
    channel = array_get(network->channels, i);
    fprintf(csv_channel_output, "%ld,%ld,%ld,%ld,%ld,%ld,%d,%d,%d\n", channel->id, channel->edge1, channel->edge2, channel->node1, channel->node2, channel->capacity, channel->is_closed, channel->is_unbalanced, channel->unbalancing_time);
  }
...
int main(int argc, char *argv[]) {
  ...
  run_dijkstra_threads(network, payments, 0, simulation);
