...
final_result_dir="result-$3"

rm -r $final_result_dir
...
do
    ...
    do
        ...
        result_dir="run-n$N_NODES-sparse$SPARSE_COEF-skew$MRATE_SKEW"
        ...
        do
            ...
            mv ./cloth/output_dir/channels_output.csv $final_result_dir/$result_dir/$CHANNEL_DATA_FOLDER/$file_name

            echo "----------------------------data-number-$i-generated----------------------------------"
        done 
        python3 real_lifeTime_aggrigator.py ./$final_result_dir/$result_dir/$CHANNEL_DATA_FOLDER $n_channels 1

        python3 error_calculator.py > errors.txt

        cp ./result_sparse_ub.ipynb ./$final_result_dir/$result_dir/result_sparse_ub.ipynb
        cp ./errors.txt ./$final_result_dir/$result_dir/errors.txt
        cp ./lifeTimes_real.csv ./$final_result_dir/$result_dir/lifeTimes_real.csv
        cp ./lifeTimes.csv ./$final_result_dir/$result_dir/lifeTimes.csv
        cp ./Mrates.csv ./$final_result_dir/$result_dir/Mrates.csv
        cp ./DataGenerator.sh ./$final_result_dir/$result_dir/DataGenerator.sh
        cp ./cloth/cloth_input.txt ./$final_result_dir/$result_dir/cloth_input.txt
        cp ./channels.csv ./$final_result_dir/$result_dir/channels.csv
